...
def parse_args():
    ...
    parser.add_argument('--img', dest='img_path', help='image path',
                        default='~/kitti_data/rt3d/data/VOCdevkit2007/VOC2007/JPEGImages/', type=str)
    parser.add_argument('--fig', dest='fig', help='save image',
                        default=False, type=bool)
    parser.add_argument('--debug', dest='debug', help='print debug info',
                        default=False, type=bool)
    ...
    parser.add_argument('--caffemodel', dest='caffemodel', help='model name',
                        default='resnet50_rfcn_ohem_iter_20000train', type=str)
...
if __name__ == '__main__':
    ...
    im = 128 * np.ones((600, 600, 3), dtype=np.uint8)
    for i in xrange(2):
        _, _, _, _= im_detect(net, im)

    # img_idx = 0
    # DIRS = os.listdir(args.img_path)
    # DIRS.sort()


    with open(os.path.join(imageset, 'Main_Chen', 'val.txt')) as test_list_file:
    ...
    for line in lines[0:]:
        ...
        if not os.path.exists(os.path.join(output, caffemodelname)):
            os.mkdir(os.path.join(output, caffemodelname))
            os.mkdir(os.path.join(output, caffemodelname, 'pred'))
            os.mkdir(os.path.join(output, caffemodelname, 'data'))

        res_name = os.path.join(output, caffemodelname, 'pred', line + '.txt')

        with open(res_name, 'w') as fp:
            car_detect(net, im_name, fp)
            if save_image:
                plt.savefig(os.path.join(output, caffemodelname, line +'.jpg'))
                plt.close('all')

    ########################################################
    ## transfer coordinator from lidar to camera
    ########################################################
    # cnnresult_dir = os.path.join(output, caffemodelname, 'pred')
    # calib_dir = '~/kitti_data/kitti_cloud/training/calib'
    # kittiresult_dir = os.path.join(output, caffemodelname, 'data')
    # eval_list = os.path.join(imageset, 'Main_Chen', 'val.txt')
    # sio.savemat('../output/mat/eval_path_%s.mat' % args.caffemodel,
